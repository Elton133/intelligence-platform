  ...
  return (
    <header className="h-16 border-b border-white/5 bg-white/1 backdrop-blur-xl flex items-center justify-between px-4 md:px-6 shrink-0 z-10 gap-4">
      ...
      <button 
        ...
        className="md:hidden p-2 -ml-2 text-white/50 hover:text-white transition-colors rounded-lg hover:bg-white/5"
      >
        <Menu01Icon size={24} />
      </button>
      ...
      <div className="flex-1 max-w-md relative group cursor-text" onClick={() => setCommandMenuOpen(true)}>
        ...
        <div className="w-full bg-white/2 border border-white/5 rounded-full py-2 pl-10 pr-4 text-sm font-light text-white/50 group-hover:border-white/20 group-hover:bg-white/4 transition-all flex items-center justify-between">
          <span className="hidden sm:inline">Search projects, sectors, or countries...</span>
          <span className="sm:hidden">Search...</span>
          <span className="hidden sm:inline-block text-[10px] font-mono text-white/20 px-1.5 py-0.5 rounded border border-white/10 bg-white/5">
            ⌘K
          </span>
        </div>
      </div>
